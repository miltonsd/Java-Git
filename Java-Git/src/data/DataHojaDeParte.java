...
public class DataHojaDeParte {
	...
	public ArrayList<entities.HojaDeParte> GetAll() {
		...
		try {
			pst = FactoryConexion.getInstancia().getConn().prepareStatement("select * from hojas_de_parte ");
			...
			if (rs != null) {
				while (rs.next()) {
					...
					hoja.setDetalle(rs.getString("detalle"));
					hojas.add(hoja);
	...
	public ArrayList<entities.HojaDeParte> getAllFactura(int id_factura) {
		...
		try {
			pst = FactoryConexion.getInstancia().getConn()
					.prepareStatement("select * from hojas_de_parte where id_factura = ?");
	...
	public entities.HojaDeParte GetOne(int idhoja) {
		...
		try {
			pst = FactoryConexion.getInstancia().getConn()
					.prepareStatement("select * from hojas_de_parte where id_hoja = ?");
			pst.setInt(1, idhoja);
			rs = pst.executeQuery();
			if (rs != null && rs.next()) {
				hoja.setID(rs.getInt("id_hoja"));
	...
	public void Update(entities.HojaDeParte hoja) {
		...
		try {
			stmt = FactoryConexion.getInstancia().getConn().prepareStatement(
					"UPDATE hojas_de_parte SET costo_mano_de_obra=?,id_mecanico=?,id_factura=?,id_patente=?,detalle=? where id_hoja = ?");
			...
			stmt.setString(5, hoja.getDetalle());
			stmt.setInt(6, (int) hoja.getID());
			stmt.execute();
	...
	public void Insert(entities.HojaDeParte hoja) {
		...
		try {
			stmt = FactoryConexion.getInstancia().getConn().prepareStatement(
					"insert into hojas_de_parte( costo_mano_de_obra, id_mecanico, id_factura, id_patente,id_hoja,detalle) values(?,?,?,?,?,?)",
			...
			stmt.setString(6, (String) hoja.getDetalle());
			stmt.executeUpdate();
